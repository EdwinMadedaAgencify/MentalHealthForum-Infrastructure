...
-- Description: Complete schema including all current and future features
-- =====================================================================
...
-- =====================================================================
-- PART 2: ENUMS - All enum types in alphabetical order
-- =====================================================================

-- ---------------------------------------------------------------------
-- 2.1 Connection Status Enum (for user connections)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.2 Content Warning Enum (for posts and threads)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.3 Dismissal Reason Enum (for report dismissals)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.4 Edit Reason Enum (for post/thread edits)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.5 Groups Enum (Keycloak groups)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.6 Moderation Action Enum (all possible moderation actions)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.12 Queue Source Enum (for moderation queue)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.13 Reaction Enum (expanded emotional reactions)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
-- ---------------------------------------------------------------------
-- 2.24 Visibility Enum (for moderation logs)
-- ---------------------------------------------------------------------
DO $$ BEGIN
...
CREATE INDEX IF NOT EXISTS idx_app_users_active ON app_users (is_active ASC, last_active_at DESC);
...
CREATE TABLE IF NOT EXISTS category_tag_assignments (
    ...
    assigned_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                                                                       CONSTRAINT category_tag_assignments_pkey PRIMARY KEY (id),
    CONSTRAINT uq_category_tag UNIQUE (category_id, tag_id)
    );

-- Indexes for category_tag_assignments
...
CREATE TABLE IF NOT EXISTS thread_edit_history (
    ...
    CONSTRAINT thread_edit_history_pkey PRIMARY KEY (id)
    );

-- Indexes for thread_edit_history
...
-- ---------------------------------------------------------------------
-- 6.3 Forum Threads - Best Answer FK (added after posts exist)
-- Description: Circular dependency fix - best_answer_post_id references posts
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS reaction_definitions (
    ...
    sort_order         INTEGER                     DEFAULT 0 NOT NULL,
    created_at         TIMESTAMP WITH TIME ZONE    DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                     CONSTRAINT reaction_definitions_pkey PRIMARY KEY (reaction_type)
    );
...
CREATE TABLE IF NOT EXISTS user_report_history (
    ...
    created_at         TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                                                                                                                               CONSTRAINT user_report_history_pkey PRIMARY KEY (user_id)
    );
...
CREATE TABLE IF NOT EXISTS moderation_action_templates (
    ...
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                  CONSTRAINT moderation_action_templates_pkey PRIMARY KEY (id)
    );

-- Seed data (subset - full list would be too long for this comment)
...
CREATE TABLE IF NOT EXISTS dismissal_reason_templates (
    ...
    created_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at      TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                  CONSTRAINT dismissal_reason_templates_pkey PRIMARY KEY (id)
    );
...
-- =====================================================================
-- PART 9: MODERATION (Future Features - Placeholder)
-- =====================================================================

-- ---------------------------------------------------------------------
-- 9.1 moderation_queue - System/AI flags (FUTURE)
...
COMMENT ON TABLE moderation_queue IS 'Queue for AI and system-flagged content awaiting moderator review';

-- ---------------------------------------------------------------------
-- 9.2 moderation_rules - Automated moderation rules (FUTURE)
...
COMMENT ON TABLE moderation_rules IS 'Rules for automated moderation actions (future feature)';

-- ---------------------------------------------------------------------
-- 9.3 moderation_tiers - Role-based permissions (FUTURE)
...
CREATE TABLE IF NOT EXISTS moderation_tiers (
    ...
    CONSTRAINT moderation_tiers_pkey PRIMARY KEY (tier_name)
    );
...
COMMENT ON TABLE moderation_tiers IS 'Defines permissions for each moderator tier';

-- ---------------------------------------------------------------------
-- 9.4 moderation_log - Full audit trail (FUTURE)
...
COMMENT ON TABLE moderation_log IS 'Complete audit trail of all moderation actions';

-- ---------------------------------------------------------------------
-- 9.5 warning_type_definitions - Warning types (FUTURE)
...
COMMENT ON TABLE warning_type_definitions IS 'Reference data for warning types';

-- ---------------------------------------------------------------------
-- 9.6 user_warnings - User warnings (FUTURE)
...
COMMENT ON TABLE user_warnings IS 'Tracks warnings issued to users';

-- ---------------------------------------------------------------------
-- 9.7 user_restrictions - User restrictions (FUTURE)
...
COMMENT ON TABLE user_restrictions IS 'Tracks mutes, suspensions, and bans';
...
-- ---------------------------------------------------------------------
-- 10.1 role_configurations - Role capabilities (FUTURE)
...
COMMENT ON TABLE role_configurations IS 'Defines capabilities for each role';

-- ---------------------------------------------------------------------
-- 10.2 group_configurations - Group capabilities (FUTURE)
...
COMMENT ON TABLE group_configurations IS 'Defines capabilities for each group';
...
CREATE TABLE IF NOT EXISTS thread_bookmarks (
    ...
    CONSTRAINT uq_user_thread_bookmark UNIQUE (user_id, thread_id)
    );
...
CREATE TABLE IF NOT EXISTS user_connections (
    ...
    CONSTRAINT uq_user_connection UNIQUE (user_1, user_2),
...
CREATE TABLE IF NOT EXISTS focus_categories (
    ...
    notification_enabled BOOLEAN                  DEFAULT FALSE NOT NULL,
    created_at           TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                                                                                  CONSTRAINT focus_categories_pkey PRIMARY KEY (id),
    CONSTRAINT uq_user_focus_category UNIQUE (user_id, category_id)
    );
...
CREATE TABLE IF NOT EXISTS watch_threads (
    ...
    notification_enabled BOOLEAN                  DEFAULT FALSE NOT NULL,
    created_at           TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP NOT NULL,
                                                                                                  CONSTRAINT watch_threads_pkey PRIMARY KEY (id),
    CONSTRAINT uq_user_watch_thread UNIQUE (user_id, thread_id)
    );
...
-- =====================================================================
-- PART 12: NOTIFICATIONS (FUTURE)
-- =====================================================================

-- ---------------------------------------------------------------------
-- 12.1 notifications - User notifications (FUTURE)
...
-- =====================================================================
-- PART 13: TRIGGERS & FUNCTIONS
-- =====================================================================

-- ---------------------------------------------------------------------
-- 13.1 Category Depth Validation - Prevent multi-level hierarchy
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.2 Category Tags Updated At - Auto-update timestamp
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.3 Post Word Count - Auto-calculate word count
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.4 Post Depth Validation - Prevent nested replies
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.5 Update Thread on Post - Maintain thread activity
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.6 Update Post Reaction Count - Maintain reaction count
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.7 Update Author Reputation - Award reputation for reactions
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.8 Report Last Modified - Auto-update timestamp
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.10 Flag Content on Report - Auto-flag reported content
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.11 Update User Connections Timestamp - Auto-update
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.12 Notification on Reply - Future feature trigger
-- ---------------------------------------------------------------------
...
COMMENT ON FUNCTION notify_on_reply() IS 'Creates notifications when users reply to posts/threads';

-- ---------------------------------------------------------------------
-- 13.13 Moderation Action Templates Updated At
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.14 Report Templates Updated At
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 13.15 Cleanup Functions - Expire old records
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14.1 user_active_restrictions - Active restrictions view
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14.2 user_warning_counts - Warning summary view
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14.3 user_connection_counts - Connection statistics view
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14.4 trending_threads - Trending threads view
-- ---------------------------------------------------------------------
...
-- ---------------------------------------------------------------------
-- 14.5 user_category_activity - Category activity view
-- ---------------------------------------------------------------------
...
-- =====================================================================
-- PART 15: FINAL VALIDATION
-- =====================================================================

-- ---------------------------------------------------------------------
-- 15.1 Schema Validation Query
-- ---------------------------------------------------------------------
DO $$
DECLARE
table_count INTEGER;
...
RAISE NOTICE 'Schema Validation Complete!';
    ...
    RAISE NOTICE '----------------------------------------';
    RAISE NOTICE '✅ Database schema ready!';
END $$;
